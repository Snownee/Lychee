...
public class LycheeRecipeType<C extends LycheeRecipeContext, T extends LycheeRecipe<C>> implements RecipeType<T> {
	...
	public <D extends Container> Optional<T> tryMatch(Recipe<D> pRecipe, Level pLevel, D pContainer) {
		...
		return pRecipe.matches(pContainer, pLevel) && lycheeRecipe.checkConditions(lycheeRecipe, (C) pContainer, 1) > 0
	...
	public List<T> inViewerRecipes() {
		return CommonProxy.recipes(this).stream().filter(OldLycheeRecipe::showInRecipeViewer).toList();
	}
	...
	public Component getPreventDefaultDescription(OldLycheeRecipe<?> recipe) {
