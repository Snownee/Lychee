...
public interface RVHelper {
	/**
	 * @param stack The stack to open
	 * @param usageOrRecipe true for usage, false for recipes
	 * @return the page is opened
	 */
	boolean openPage(ItemStack stack, boolean usageOrRecipe);

	default boolean openPage(Block block, boolean usageOrRecipe) {
		return openPage(block.asItem().getDefaultInstance(), usageOrRecipe);
	}

	default boolean openPage(BlockState state, boolean usageOrRecipe) {
