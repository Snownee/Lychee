...
public class BlockCrushingRecipeCategory extends AbstractRvCategory<BlockCrushingRecipe> {
	...
	@Override
	public void configureLayout(RecipeHolder<BlockCrushingRecipe> recipeHolder, Vector2i position) {
		...
		var layoutBuilder = Objects.requireNonNull(rvHandler().layoutBuilder, "Missing layout builder");
		layoutBuilder.ingredientGroup(recipe, new Vector2i(centerX - 45, y));
		layoutBuilder.actionGroup(recipe, new Vector2i(centerX + 50, y));
	}

	@Override
	public void configureDecorations(RecipeHolder<BlockCrushingRecipe> recipeHolder, Vector2i position) {
		var recipe = recipeHolder.value();
		var widgetBuilder = Objects.requireNonNull(rvHandler().widgetBuilder, "Missing decoration builder");
		...
		widgetBuilder.addElement(RenderElement.create((graphics, x, y) -> {
		...
		widgetBuilder.addElement(new InteractiveRenderElement()
				.<InteractiveRenderElement>at(fallingBlockPosition.x(), fallingBlockPosition.y())
				.<InteractiveRenderElement>withSize(FALLING_BLOCK_SIZE.x(), FALLING_BLOCK_SIZE.y())
		...
		if (!landingBlockIsAny) {
			widgetBuilder.addElement(new InteractiveRenderElement()
					.<InteractiveRenderElement>at(landingBlockPosition.x(), landingBlockPosition.y())
					.<InteractiveRenderElement>withSize(LANDING_BLOCK_SIZE.x(), LANDING_BLOCK_SIZE.y())
		...
		if (AbstractRvCategory.needRemoveInputIcon(recipe)) {
			...
			AbstractRvCategory.addRemoveInputIcon(widgetBuilder, removeActionPosition);
		}
	}
