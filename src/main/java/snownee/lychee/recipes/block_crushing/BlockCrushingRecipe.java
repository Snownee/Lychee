...
public class BlockCrushingRecipe extends OldLycheeRecipe<BlockCrushingContext>
	...
	public boolean matches(BlockCrushingContext ctx, Level pLevel) {
		...
		if (!BlockPredicateHelper.matches(landingBlock, ctx)) {
			return false;
		}
	...
	public int compareTo(BlockCrushingRecipe that) {
		...
		if (i != 0)
			return i;
		i = Integer.compare(
				landingBlock == BlockPredicate.ANY ? 1 : 0,
				that.landingBlock == BlockPredicate.ANY ? 1 : 0
		);
		if (i != 0)
			return i;
	...
	public static class Serializer extends OldLycheeRecipe.Serializer<BlockCrushingRecipe> {
