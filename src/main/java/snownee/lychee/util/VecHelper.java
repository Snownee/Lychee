...
public class VecHelper {
	...
	public static boolean isVecPointingTowards(Vec3 vec, Direction direction) {
		return Vec3.atLowerCornerOf(direction.getNormal()).dot(vec.normalize()) > 0.125; // slight tolerance to activate perpendicular movement actors
	}
	...
	public static Vec3 offsetRandomly(Vec3 vec, Random r, float radius) {
		return new Vec3(vec.x + (r.nextFloat() - .5f) * 2 * radius, vec.y + (r.nextFloat() - .5f) * 2 * radius, vec.z + (r.nextFloat() - .5f) * 2 * radius);
	}
	...
	public static Vec3 clamp(Vec3 vec, float maxLength) {
		return vec.length() > maxLength ? vec.normalize().scale(maxLength) : vec;
	}
	...
	public static Vec3 projectToPlayerView(Vec3 target, float partialTicks) {
		...
		if (mc.options.bobView().get()) {
			...
			if (renderViewEntity instanceof Player) {
				...
				Quaternion q2 = new Quaternion(Vector3f.XP, Math.abs(Mth.cos(f1 * (float) Math.PI - 0.2F) * f2) * 5.0F, true);
				q2.conj();
